	 ...
	 <div class="container-fluid">
		...
		<div class="row-fluid">
		   <div class="span12">
			  ...
			  <div class="alert alert-success">
				<button data-dismiss="alert" class="close"></button>
				<strong>Success!</strong> equipment updated
			  </div>
						...
						<div class="portlet-body">
							<!-- <div class="clearfix">
								<div class="btn-group">
									...
									<a class="btn mini green" href="manageequipment.php?mode=add">Add New <i class="icon-plus"></i></a>
								...
								<div class="btn-group pull-right">
									...
									<ul class="dropdown-menu">
										<li><a onclick="delselected('data','equipment')" style="cursor:pointer">Delete Selected</a></li>
									</ul>
								</div>
							</div> -->
							<div id="tablesec">
							<table class="table table-striped table-bordered table-hover" id="sample_1">
								...
								<tbody>
									...
									<?php
									$getdest = "SELECT * FROM ".TABLE_PREFIX."booking ORDER BY id DESC";
									$getdest = mysqli_query($conn,$getdest) or die (mysqli_error($conn));
									while($rowdest = mysqli_fetch_array($getdest))
									{
										$slot = implode('<br>', json_decode($rowdest['booking_slot']));
									?>
									...
									<tr class="odd gradeX">
										...
										<td class="hidden-480"><?=stripslashes($rowdest['email'])?></td>
										<td class="hidden-480"><?=stripslashes($rowdest['phone'])?></td>
										<td class="hidden-480"><?=stripslashes($rowdest['booking_date'])?></td>
										<td class="hidden-480"><?=$slot;?></td>
										
										
										<td class="hidden-480">
																				   
										   <div class="controls">
											 <select class="span6 chosen" tabindex="1" id="stat<?=$rowdest['id']?>" onChange="changestatus(this.value,'<?=$rowdest['id']?>','<?=$rowdest['phone']?>')">
											 	<option value="0" <?=$rowdest['status'] == '0' ? 'selected' : ''?>>Select One</option>
												<option value="1" <?=$rowdest['status'] == '1' ? 'selected' : ''?>>Confirm</option>
												<option value="2" <?=$rowdest['status'] == '2' ? 'selected' : ''?>>Cancel</option>
											 </select>
										  </div>
...
<script>
	...
	function changestatus(status,id,phone)
	{
		//alert(phone);
		//alert(stat);
		//var table = "data";
		$.post('ajax/cofirmbooking.php',{ status : status, id : id, phone : phone });
	}
</script>
