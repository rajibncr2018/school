...
if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'add')
{
	...
	header("location:subcategory.php?action=added");
}
...
if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit')
{
	...
	header("location:subcategory.php?action=updated");
}
?>
						...
						<div class="portlet-body form">
						   ...
						   <div class="control-group">
						    ...
						    <div class="control-group">
							  ...
							  <div class="controls">
							  	<select name="category_id" id="category_id" class="span6 m-wrap">
							  		<?php
									$getdest = "SELECT * FROM ".TABLE_PREFIX."data WHERE type = 'category' ORDER BY id DESC";
									$getdest = mysqli_query($conn,$getdest) or die (mysqli_error($conn));
									while($rowdest = mysqli_fetch_array($getdest))
									{
									?>
							  		<option value="<?=$rowdest['id'];?>" <?=$rowdest['id']==$getdata['category_id'] ? "selected" : "";?>><?=$rowdest['name'];?></option>
							  		<?php
							  		}
							  		?>
							  	</select>
						   ...
						   <div class="control-group">
								  ...
								  <div class="controls">
									<textarea class="span12 ckeditor m-wrap" name="project_info" rows="6" id="project_info"><?=isset($fetchdata->project_info) && $fetchdata->project_info ? stripslashes($fetchdata->project_info) : "";?></textarea>
								  </div>
							   </div>
							   ...
							   <div class="control-group">
								  <label class="control-label">Project Features</label>				  
								  <div class="controls">
									<textarea class="span12 ckeditor m-wrap" name="project_features" rows="6" id="project_features"><?=isset($fetchdata->project_features) && $fetchdata->project_features ? stripslashes($fetchdata->project_features) : "";?></textarea>
								  </div>
							   </div>
